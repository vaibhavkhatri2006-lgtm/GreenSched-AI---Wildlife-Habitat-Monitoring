...
export default function Analysis() {
 ...
 <h2 className="font-heading font-semibold text-2xl text-accent mb-4 tracking-wider">Run New Analysis</h2>
 ...
 <div>
 <label className="block text-[10px] text-text-secondary mb-1">Baseline Start</label>
 <input type="date" value={baselineStart} onChange={(e) => setBaselineStart(e.target.value)} className="w-full bg-surface-1 border border-successorder rounded-md px-2 py-1.5 text-xs text-text" />
 </div>
 <div>
 <label className="block text-[10px] text-text-secondary mb-1">Baseline End</label>
 <input type="date" value={baselineEnd} onChange={(e) => setBaselineEnd(e.target.value)} className="w-full bg-surface-1 border border-successorder rounded-md px-2 py-1.5 text-xs text-text" />
 </div>
 </div>
 <div className="grid grid-cols-2 gap-2">
 <div>
 <label className="block text-[10px] text-text-secondary mb-1">Current Start</label>
 <input type="date" value={currentStart} onChange={(e) => setCurrentStart(e.target.value)} className="w-full bg-surface-1 border border-successorder rounded-md px-2 py-1.5 text-xs text-text" />
 </div>
 <div>
 <label className="block text-[10px] text-text-secondary mb-1">Current End</label>
 <input type="date" value={currentEnd} onChange={(e) => setCurrentEnd(e.target.value)} className="w-full bg-surface-1 border border-successorder rounded-md px-2 py-1.5 text-xs text-text" />
 </div>
 </div>
 ...
 {status && <p className="text-xs text-center text-text-secondary">{status}</p>}
 </div>
 </div>

 {/* AI Insights Box */}
 <div className="glass-panel p-6 rounded-2xl border border-successorder border-l-4 border-l-[#F4C430] shadow-xl">
 <h2 className="font-heading font-semibold text-xl text-accent mb-6 flex items-center gap-3">
 <Brain size={20} />
 ...
 <p className="text-text-secondary italic">"{aiInsight.summary}"</p>
 
 <div>
 <h4 className="font-semibold text-accent text-sm st mb-2">Key Findings</h4>
 <ul className="list-disc pl-4 text-text-secondary text-xs space-y-1">
 {aiInsight.key_findings.map((f, i) => <li key={i}>{f}</li>)}
 </ul>
 </div>

 <div>
 <h4 className="font-semibold text-accent text-xs mb-1">Recommended Actions</h4>
 <ul className="list-disc pl-4 text-text-secondary text-xs space-y-1">
 {aiInsight.recommended_actions.map((f, i) => <li key={i}>{f}</li>)}
 </ul>
 </div>

 <div className="p-3 bg-surface-1/50 rounded text-xs text-text-secondary">
 <span className="font-semibold block mb-1">Limitations:</span>
 {aiInsight.limitations.map((l, i) => <div key={i}>• {l}</div>)}
 </div>
 </div>
 ) : (
 <div className="text-sm text-text-muted text-center py-6">Run or select an analysis to generate insights.</div>
 )}
 </div>
 ...
 <div className="glass-panel p-4 rounded-xl border border-successorder">
 <div className="text-xs text-text-secondary mb-1">NDVI Change</div>
 <div className={`text-2xl font-bold ${analysisData.ndvi_change < 0 ? 'text-danger' : 'text-accent'}`}>
 {analysisData.ndvi_change > 0 ? '+' : ''}{(analysisData.ndvi_change*100).toFixed(2)}%
 </div>
 </div>
 <div className="glass-panel p-4 rounded-xl border border-successorder">
 <div className="text-xs text-text-secondary mb-1">NDWI Change</div>
 <div className="text-2xl font-bold text-accent">
 {analysisData.ndwi_change > 0 ? '+' : ''}{(analysisData.ndwi_change*100).toFixed(2)}%
 </div>
 </div>
 <div className="glass-panel p-4 rounded-xl border border-successorder">
 <div className="text-xs text-text-secondary mb-1">Urban Expansion</div>
 <div className={`text-xl font-bold ${analysisData.builtup_change_area_km2 > 0 ? 'text-accent' : 'text-text-secondary'}`}>
 +{analysisData.builtup_change_area_km2?.toFixed(2)} km²
 </div>
 </div>
 <div className="glass-panel p-4 rounded-xl border border-danger/30 bg-danger/5">
 <div className="text-xs text-danger mb-1 font-bold">Hotspots Detected</div>
 <div className="text-xl font-bold text-danger">{analysisData.hotspot_count}</div>
 </div>
 </div>
 )}

 {/* Map */}
 <div className="flex-1 glass-panel rounded-xl border border-successorder overflow-hidden relative min-h-[500px]">
 ...
 <span className="w-3 h-3 rounded-sm bg-success opacity-60"></span>
 ...
 <span className="text-text-secondary">Vegetation Loss Hotspot</span>
 </div>
 <div className="flex items-center gap-2">
 <span className="w-3 h-3 rounded-full bg-purple-500 shadow-[0_0_8px_#a855f7]"></span>
 <span className="text-text-secondary">Urban Expansion Hotspot</span>
 </div>
 <div className="flex items-center gap-2">
 <span className="w-3 h-3 rounded-full bg-info shadow-[0_0_8px_#06b6d4]"></span>
 <span className="text-text-secondary">Water Body Change Hotspot</span>
 </div>
 <div className="flex items-center gap-2">
 <span className="w-3 h-3 rounded-full bg-danger shadow-[0_0_8px_#ef4444]"></span>
 <span className="text-text-secondary">Unspecified Critical Hotspot</span>
 </div>
 </div>
 </div>

 <MapContainer center={[-2.3333, 34.8333]} zoom={5} className="h-full w-full bg-successg z-0">
 {showSatellite && (
 <TileLayer
